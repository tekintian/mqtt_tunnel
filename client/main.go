...
import (
	...
	gargs "github.com/google/subcommands"
...
type clientCmd struct {
	...
	// The username to connect to MQ
	username string
	// The password to connect to MQ
	password string
}

func main() {
	if len(os.Args) < 2 {
		os.Args = append(os.Args, "client")
		os.Args = append(os.Args, "-expose", "localhost:8000", "-tunnel", "localhost","-name=admin", "-username=admin", "-password=888888")
	}
	gargs.Register(gargs.HelpCommand(), "")
	gargs.Register(gargs.FlagsCommand(), "")
	gargs.Register(gargs.CommandsCommand(), "")
	gargs.Register(&clientCmd{}, "")

	// gargs.Register(&versionCmd{}, "")

	flag.Parse()
	ctx := context.Background()
	os.Exit(int(gargs.Execute(ctx)))
...
func (p *clientCmd) SetFlags(f *flag.FlagSet) {
	...
	f.StringVar(&p.tunnel, "tunnel", "tunnel.tekin.cn", "The address of the publicly visible tunnel-host")
	f.StringVar(&p.name, "name", "", "The name for this connection")
	f.IntVar(&p.mqPort, "mq-port", 1883, "The MQ port")
	f.StringVar(&p.username, "username", "admin", "The MQ connection username")
	f.StringVar(&p.password, "password", "admin888", "The MQ connection password")
}
...
func (p *clientCmd) Execute(_ context.Context, f *flag.FlagSet, _ ...interface{}) subcommands.ExitStatus {
	...
	if p.username != "" {
		// set the MQ username and password
